...
- (void)setView
{
    ...
    CGFloat firstButtonOriginY = bgView.frame.size.height - buttonHeight - space*2;
    ...
    for (int i = 0; i < countButtonTitleArray; i++) {
        ...
        button.tag = i + 1;
        button.frame = CGRectMake(space, buttonOriginY, bgView.frame.size.width - space*2, buttonHeight);
    ...
    if (_isCancelButton == YES) {
        ...
        leftButton.frame = CGRectMake(space, bgView.frame.size.height - buttonHeight - space, bgView.frame.size.width - space*2, buttonHeight);
